...
from procedure import create_ranges
import os
...
result_path = "C:/Users/plexa/OneDrive/Bayreuth/LSS5-LSS20/diffPy/multiple_datasets/fit_results/boxcar_fits/"
...
def run():
    ...
    ranges = create_ranges(1.8, 30, 2, multiplyer=5)
    ranges = [(1.8,8),(5,15),(10,20),(15,25),(20,30),(25,35)]
    for name, path in datasets.items():
        ...
        LSS_cif = cifs[name]
        for range in ranges:
            # Create a modified config for this range

            range_config = CONFIG.copy()
            range_config["PDF_RMIN"], range_config["PDF_RMAX"] = range

            recipe = make_recipe(LSS_cif, path, config=range_config, space_group='Pnma')  # Call the function to create the fit recipe
            ...
            result_path_dir = result_path +f"{name}/"+ f"fit_{range[0]}_{range[1]}/"
